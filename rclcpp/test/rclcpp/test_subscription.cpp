...
public:
  ...
  void CreateSubscription()
  {
    auto node_ = std::make_shared<rclcpp::Node>("test_subscription_member_callback", "/ns");
    auto callback = std::bind(&SubscriptionClass::OnMessage, this, std::placeholders::_1);
    using test_msgs::msg::Empty;
    auto sub = node_->create_subscription<Empty>("topic", 10, callback);
  }
};
...
TEST_F(TestSubscription, callback_bind) {
  ...
  }
  {
    ...
    auto sub = node_->create_subscription<Empty>("topic", 1, callback);
  }
}
