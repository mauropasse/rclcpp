...
GraphListener::run_loop()
{
  while (true) {
    ...
    ret = rcl_wait_set_add_guard_condition(&wait_set_, &interrupt_guard_condition_, NULL, NULL);
    if (RCL_RET_OK != ret) {
    ...
    ret = rcl_wait_set_add_guard_condition(
      &wait_set_, shutdown_guard_condition_, NULL, &shutdown_guard_condition_index);
    if (RCL_RET_OK != ret) {
    ...
    for (size_t i = 0u; i < node_graph_interfaces_size; ++i) {
      ...
      ret = rcl_wait_set_add_guard_condition(&wait_set_, graph_gc, NULL, &graph_gc_indexes[i]);
      if (RCL_RET_OK != ret) {
