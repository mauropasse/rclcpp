...
{
  rcl_guard_condition_t * timer_gc = rcl_timer_get_guard_condition(timer_handle_.get());
  rcl_ret_t ret = rcl_guard_condition_set_on_trigger_callback(timer_gc, callback, user_data);

  if (ret != RCL_RET_OK) {
    rclcpp::exceptions::throw_from_rcl_error(ret, "Failed to set on reset callback");
  }
}
