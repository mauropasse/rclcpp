...
public:
  ...
  RCLCPP_PUBLIC
  void
  add_guard_condition(GuardCondition::SharedPtr guard_condition)
  {
    ...
    const void * user_data) override
  {
    for (auto gc : notify_guard_conditions_) {
      gc->set_callback(callback, user_data);
    }
  }
...
private:
  std::list<GuardCondition::SharedPtr> notify_guard_conditions_;
};
